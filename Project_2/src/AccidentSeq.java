...
/**
* A sequence of Accident instances.
*/
public final class AccidentSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AccidentSeq (int initialMaximum) {
        super(Accident.class, initialMaximum);
    }

    public AccidentSeq (Collection elements) {
        super(Accident.class, elements);
    }

    public  Accident get(int index) { 
        return (Accident) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Accident.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Accident objects (beyond the original size of this object)

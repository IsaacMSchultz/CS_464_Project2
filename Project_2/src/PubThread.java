...
public class PubThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				positionTopic = participant.create_topic("P2464_ischultz:POS", typeName, DomainParticipant.TOPIC_QOS_DEFAULT, null /* listener */, StatusKind.STATUS_MASK_NONE);
				if (positionTopic == null) {
					System.err.println("create_topic error\n");
					return;
				}
				
				accidentTopic = participant.create_topic("P2464_ischultz:ACC", typeName, DomainParticipant.TOPIC_QOS_DEFAULT, null /* listener */, StatusKind.STATUS_MASK_NONE);
				...
				long betweenStops = routeInfo.TimeBetweenStops() * 1000; // time between stops in ms
				...
				for (int stop = 1; stop < routeInfo.Stops() + 1; ++stop) {
					...
					positionInstance.timeBetweenStops = actualWaitTime / 1000;
					
						
					
					System.out.println("Writing Position, count " + stop);
